...
namespace MoreMountains.InventoryEngine
{
    public class InventoryInputManagerExtended : InventoryInputManager
    {
        ...
        private const string _playerID = "Player1";
        ...
        protected virtual void MultiInventoryMove(bool keyboard)
        {
            ...
            else
            {
                ...
                MMInventoryEvent.Trigger(MMInventoryEventType.ContentChanged, null, _inventoryFrom.name, null, 0, 0, _playerID);
                MMInventoryEvent.Trigger(MMInventoryEventType.ContentChanged, null, _inventoryTo.name, null, 0, 0, _playerID);
            }
        }
                    ...
                    if (openedVendor.canRollBackItems)
                    {
                        if (openedVendor.vendorRollBackInventory.AddItem(item, quantity))
                        {
                            if (openedVendor.mainInventory.AddItem(openedVendor.currencyItem, price))
                            {
                                ...
                                MMInventoryEvent.Trigger(MMInventoryEventType.ContentChanged, null, openedVendor.vendorRollBackInventory.name, null, 0, 0, _playerID);
                                ...
                                MMInventoryEvent.Trigger(MMInventoryEventType.ContentChanged, null, openedVendor.mainInventory.name, null, 0, 0, _playerID);
                            }
                        }
                    }
                    ...
                    if (cash >= price)
                    {
                        ...
                        if (inventory.name == openedVendor.vendorInventory.name)
                        {
                            ...
                            if (openedVendor.limitedStock)
                            {
                                ...
                                MMInventoryEvent.Trigger(MMInventoryEventType.ContentChanged, null, openedVendor.vendorInventory.name, null, 0, 0, _playerID);
                            }
                        }
